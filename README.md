...
An offline version of the tool is now available, which won't load PowerView from the internet (useful against web filtering). PowerView is instead encrypted within the script and will decrypt and load in memory, therefore it shouldn't trigger AVs when you drop it on disk (tested against Defender and Sophos).
